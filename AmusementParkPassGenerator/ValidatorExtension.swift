...
extension Validator {
    ...
    func contractPassValidation(info: Info?) throws {
        guard let providedInfo = info else {
            return
        }
        
        if isValid(infoName: .ssn, info: providedInfo.ssn) {
            throw DataError.ssnMissing
        }
        
        if isValid(infoName: .projectNumber, info: providedInfo.projectNumber) {
            throw DataError.projectNumberMissing
        }
